...
class Authentification:
    ...
    def login(prenom, mdp):
        user = EntropyModel.query.filter_by(prenom=prenom).first()
    ...
    def register(prenom, nom, mdp, calculer_entropy):
        ...
        existing_user = EntropyModel.query.filter_by(prenom=prenom).first()
        if existing_user:
        ...
        new_user = EntropyModel(prenom=prenom, nom=nom, mdpHache=hashed_mdp, entropy=ent_calc)
